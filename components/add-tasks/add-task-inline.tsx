...
export default function AddTaskInline({
  ...
  // Use parent's value if available; otherwise, use a default fallback.
  const projectId =
    myProjectId ||
    parentTask?.projectId ||
    (GET_STARTED_PROJECT_ID as Id<"projects">);
  const labelId = parentTask?.labelId; // may be undefined
  ...
  // Once both projects and labels are loaded, update form defaults.
  useEffect(() => {
    if (projects.length > 0 && labels.length > 0) {
      const effectiveProjectId =
        myProjectId || parentTask?.projectId || projects[0]._id;
      const effectiveLabelId = parentTask?.labelId || labels[0]._id;
      form.reset({
        ...baseDefaultValues,
    ...
    // We intentionally do not add `parentTask`/`myProjectId` in the dependency array if they are static.
  }, [projects, labels]);
