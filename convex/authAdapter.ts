...
export const createUser = adapterMutation({
  ...
  // 1. Insert the new user into the `users` table
   const userId = await ctx.db.insert("users", user as any);

   // 2. Create a default project for the new user
   await ctx.db.insert("projects", {
    userId,
    name: "Get Started", // or any default name you prefer
    type: "user",        // your schema requires either "user" or "system"
  });

   // 3. Create some default labels for the new user
   const defaultLabels = ["Personal", "Work", "Errands", "Finances", "Health", "Travel", "AI"];
   for (const labelName of defaultLabels) {
     await ctx.db.insert("labels", {
       userId,
       name: labelName,
       type: "user", // your schema: type is either "user" or "system"
     });
   }
